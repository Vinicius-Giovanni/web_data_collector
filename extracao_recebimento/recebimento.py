...
@log_with_context(job='data_extraction_recebimento', logger=logger)
def data_extraction_recebimento(cookies: list[dict],
                                download_dir: Path,
                                list_filial: list,
    ...
    try:
        ...
        for filial_value in list_filial:
            ...
            driver.get(LINKS['LOGIN_RECEBIMENTO'])

            logger.info(f'extraindo relatorio 1.06 - Recebimento da filial: {filial_value}', extra={'status':'iniciado'})
            ...
            logger.info('iniciando extracao do relatorio 1.06 - Recebimento', extra={'status':'iniciado'})

            filial = wait.until(EC.element_to_be_clickable(
                (By.ID, ELEMENTS['ELEMENTS_RECEBIMENTO']['element_filial_id'])
            ))
            if filial:
                Select(filial).select_by_value(filial_value)

            dt_start = wait.until(EC.element_to_be_clickable(
                (By.ID, ELEMENTS['ELEMENTS_RECEBIMENTO']['element_dt_start'])
            ))
            if dt_start:
                dt_start.clear()
            ...
            dt_end = wait.until(EC.element_to_be_clickable(
                (By.ID, ELEMENTS['ELEMENTS_RECEBIMENTO']['element_dt_end'])))
            if dt_end:
                dt_end.clear()
                dt_end.send_keys(exit_date)

            confirmar = wait.until(EC.element_to_be_clickable(
                (By.ID, ELEMENTS['ELEMENTS_RECEBIMENTO']['element_confirm'])))
            if confirmar:
                confirmar.click()
            
            if wait_download_csv(dir=TEMP_DIR['BRONZE']['recebimento']):
                logger.info(f'relatorio 1.06 - Recebimento da filial: {filial_value} extraido com sucesso', extra={'status':'sucesso'})
            else:
                logger.critical(f'falha na extracao do relatorio 1.06 - Recebimento da filial: {filial_value}', extra={'status':'critico'})

    except Exception as e:
        logger.critical(f'erro inesperado durante a extracao: {e}', extra={'status':'critico'})
    finally:
        driver.quit()
        logger.info('instancia fechada', extra={'status':'sucesso'})


def data_extraction_recebimento_from_file(cookies_path: str,
                                          download_dir: Path,
                                          list_filial: list,
                                          parquet_folder: Path | None,
                                          entry_date: str | Callable,
    ...
    data_extraction_recebimento(cookies,
                                download_dir,
                                list_filial,
                                parquet_folder,
                                entry_date,
