...
def data_extraction_cancel(cookies: list[dict],
                         download_dir: Path,
    ...
    try:
        ...
        for filial_value in ELEMENTS['ELEMENTS_CANCEL']['element_filial']:
            ...
            logger.info(f'extraindo relatorio 6.10 - Pedidos Cancelados da filial: {filial_value}', extra={'status': 'iniciado'})
            ...
            if wait_download_csv(dir=TEMP_DIR['BRONZE']['cancel'] ):
                logger.info('download do relatorio 6.10 - Pedidos Cancelados concluido', extra={'status': 'sucesso'})
            else:
                logger.warning('download do relatorio 6.10 - Pedidos Cancelados falhou', extra={'status': 'perigoso'})

    except Exception as e:
        logger.critical(f'download do relatorio 6.10 - Pedidos Cancelados falhou', extra={'status': 'critico'})

    finally:
            driver.quit()

def data_extraction_cancel_from_file(cookies_path: str,
                                    download_dir: Path,
                                    parquet_folder: Path | None,
                                    entry_date: str | Callable,
                                    exit_date: str | Callable) -> None:
